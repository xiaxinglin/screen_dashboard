...
func (gno Gnocchi)  getGnocchiTopN(token string, metric string, num int) (int, string) {
	...
	for i < lenTopn {
		...
		jsonTemp :=  fmt.Sprintf(`{"%s":%f}`, hostName, gjson.Get(jsonStr, indexStrValue).Float())
		if jsonTopN != "" {
			jsonTopN = jsonTopN + ",\n" + jsonTemp
		}else{
			jsonTopN = jsonTemp
	...
	jsonOutput := fmt.Sprintf(`{"hostState":[%s]}`, jsonTopN)
	fmt.Println(jsonStr)
...
func (gno Gnocchi)  getHostDetail(token string) (int, string)  {
	...
	for i < sum {
		...
		jsonTemp :=  fmt.Sprintf(`{"name":"%s", "score":%f}`, hostname, score)
		if jsonState != "" {
			jsonState = jsonState + ",\n" + jsonTemp
		}else{
			jsonState = jsonTemp
